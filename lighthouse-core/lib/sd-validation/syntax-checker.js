 ...
 * @param {string} input
 * @param {string} url
 * @returns {Promise<{message: string, lineNumber?: number}|null>}
 */
async function checkMicrodata(input, url) {
  try {
    await parser.parseMicrodata(input, url);
  } catch (e) {
 ...
 * @returns {Promise<{message: string, lineNumber?: number}|null>}
 */
async function checkRDFa(input, url) {
  try {
    await parser.parseRdfa(input, url);
  } catch (e) {
    return {message: e.message};
  }
  return null;
